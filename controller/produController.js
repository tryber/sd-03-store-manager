...
const getById = rescue(async (req, res) => {
  ...
  product.length ? res.status(200).json(product)
...
const createProduct = rescue(async (req, res) => {
  ...
  newProd.error ? res.status(422).json({ message: newProd.message })
...
const updateProduct = rescue(async (req, res) => {
  ...
  updProd.error ? res.status(422).json({ message: updProd.message })
